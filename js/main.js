	...
	$(document).bind("pageinit", function(){
		$.getJSON('http://elastic.restopengov.org/gcba/set-filmaciones/_search?q=Fecha%20de%20Rodaje:11/01/2011&size=10', function(data){
			for(i = 0; i < data.hits.hits.length; i++){
				var num = data.hits.hits[i]._source["Nœmero"];
				num = Math.round(num);				
				filmdire = data.hits.hits[i]._source["Calle de Locaci—n"] + " " + num + ", Ciudad Aut—noma de Buenos Aires, Argentina";
				filmtit = data.hits.hits[i]._source["T’tulo del Proyecto"];
				filmtip = data.hits.hits[i]._source["Tipo de Producci—n"];
